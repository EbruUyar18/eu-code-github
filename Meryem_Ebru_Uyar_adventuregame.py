#Meryem Ebru UYAR - 24370031042 
# python ile metin tabanlı Oyun geliştirme projem
"""
...
def yavas_yaz(metin): #time modülünden yararlanarak yavas yazma efektli fonk. 
    for harf in metin:
        print(harf, end='', flush=True)
        time.sleep(0.03)
    print()

#Karakter bilgilerini ve ipucu listesini depo ettiğim sınıf
class Karakter: 
...
#oyuncuya verilen görev için tanımlanan fonk
...
def oyunu_baslat():
    ...
    while True:
        ...
        if cevap == "e":
            ...
            print("Tekrar dene...")
#burada fonksiyonları çağırarak kullanılabilir hale getirdim
    giris(oyuncu)
    orman_gorevi(oyuncu)
    secim(oyuncu)
    son_gorev(oyuncu)
